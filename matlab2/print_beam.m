...
%% print beam
	...
	filename = strcat(outpath,'beam_chars.dat');
	fileID = fopen(filename,'w');
	for k4 = 1 : (length(y)-1)
	fprintf(fileID,'%E \t %E \t %E \t %E \t %E \n',y(k4), r(k4), rmiro(k4), r_stat(k4), r_E(k4) );
	end
	fprintf(fileID,'%E \t %E \t %E \t %E \t %E ',y(length(y)), r(length(y)), rmiro(length(y)), r_stat(length(y)), r_E(length(y)) );
	fclose(fileID);
