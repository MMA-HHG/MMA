...
image.xlabel = r'$p$ [mbar]'; image.ylabel = r'XUV signal [arb. u.]'; image.right_ylabel = 'ionisation [%]'
image.title = r'$H'+str(Hgrid_study[k1]) + r'$'
...
image.sf[4].axis_location = 'right'

image.right_ylim_args = [3.7, 16]
...
image.savefig_args = ['compare1_0.pdf']
...
image.sf[6].axis_location = 'right' 

image.savefig_args = ['compare1_1.pdf']
pp.plot_preset(image)

image.sf[7].args = [p_grid, 100*ionisation_ratio_optimal(Hgrid_study[k1])*np.ones(len(p_grid)), 'g--']
image.sf[7].axis_location = 'right' 

image.savefig_args = ['compare1_2.pdf']
pp.plot_preset(image)
...
image.savefig_args = ['compare1_3.pdf']
pp.plot_preset(image)
...
image.savefig_kwargs = {'dpi' : 600,'bbox_inches' : 'tight'}
image.set_fontsizes = 'doublet+'
image.title = r'$H'+str(Hgrid_study[k1]) + r'$, 40 A'
image.xlabel = r'$p$ [mbar]'; image.ylabel = r'XUV signal [arb. u.]'; image.right_ylabel = r'ionisation ($\eta$) [%]'

image.right_ylim_args = [3.5, 14]
image.ylim_args = [0, 8.6]

image.sf[0].args = [p_grid, XUV_energy_pp[0][:,0,k1]/np.mean(XUV_energy_pp[0][:,0,k1]),'k']; image.sf[0].kwargs = {'label' : 'no preion.'}    
image.sf[1].args = [p_grid, XUV_energy_pp[0][:,1,k1]/np.mean(XUV_energy_pp[0][:,0,k1]),'b']; image.sf[1].kwargs = {'label' : r'40 A'}
image.sf[3].args = [p_grid, ionisations[0]['half'], 'b--']; # image.sf[4].kwargs = {'label' : 'by discharge + transient'} 
# image.sf[3].kwargs = {'label' : r'$\eta$ num.'}

image.sf[3].axis_location = 'right'
...
image.legend_args = [custom_lines,['no preion.', '40 A', r'$\eta$ num.']]
# image.legend_kwargs = {'loc': 1, 'ncol': 2}


image.savefig_args = ['compare3_0.pdf']
pp.plot_preset(image)


A0 = A_norm(np.mean(XUV_energy_pp[0][:,0,k1]),Hgrid_study[k1],omegaSI)
image.sf[2].args = [p_grid, IntensXUV(1e-2*ionisations[0]['half_init'],Hgrid_study[k1],omegaSI,A0)/np.mean(XUV_energy_pp[0][:,0,k1]),'r'];
image.sf[2].kwargs = {'label' : r'analytic est.'}

custom_lines = [Line2D([1], [0], color="k"),
                Line2D([0], [0], color="b"),
                Line2D([0], [0], color="b", linestyle="--"),
                Line2D([0], [0], color="r")]
image.legend_args = [custom_lines,['no preion.', '40 A', r'$\eta$ num.', r'analytic est.']]

image.savefig_args = ['compare3_1.pdf']
pp.plot_preset(image)

# for k2 in range(3,8): image.sf[k2].axis_location = 'right'
 

image.sf[2].args = []; image.sf[2].kwargs = {}


preions = ionisation_ratio(A0,XUV_energy_pp[0][:,1,k1],Hgrid_study[k1],omegaSI)
image.sf[4].args = [p_grid, 100*preions[0][:], 'r--'];
image.sf[5].args = [p_grid, 100*preions[1][:], 'r--'];
image.sf[4].axis_location = 'right'
image.sf[5].axis_location = 'right'
...
image.savefig_args = ['compare3_2.pdf']
pp.plot_preset(image)
...
# image.savefig_args = ['compare3_F.pdf']
