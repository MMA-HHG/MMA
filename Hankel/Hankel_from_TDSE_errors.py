...
kr_steps = [8,4,2,1] # descending order, tha last is "the most accurate"
ko_step = 2
...
FField_FF = []
for k1 in range(len(kr_steps)):
    FSourceTerm_select = np.squeeze(FSourceTerm[0:Nr_max:kr_steps[k1],:,H_indices[0]:H_indices[1]:ko_step]).T
    
    FField_FF.append(Hfn2.HankelTransform(ogrid_select_SI,
                                     rgrid_macro[0:Nr_max:kr_steps[k1]],
                                     FSourceTerm_select,
                                     0.3,
                                     rgrid_FF)
                     )
