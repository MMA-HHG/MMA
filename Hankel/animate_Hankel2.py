...
# mpl.rcParams['animation.ffmpeg_path'] = r'C:\\utility\\ffmpeg\\bin\\ffmpeg.exe'
...
image.sf[0].args = [Hgrid, rgrid_FF, np.abs(HHG_onscreen[-1,k_z0,:,:].T) ]
...
cax = ax.pcolormesh(Hgrid, 1e3*rgrid_FF, np.abs(HHG_onscreen[0,k_z0,:,:].T), vmax=anim_vmax)
# cax = ax.pcolormesh(Hgrid, rgrid_FF, np.abs(HHG_onscreen[0,k_z0,:,:].T))
# ax.set_title(str(I0_grid[-1]))
ax.set_title("I0 = {:.2E}".format(1e-4*units.INTENSITYau*I0_grid[0]) + " W/cm2, z0={:.2E}".format(1e3*z0_grid[k_z0]) + ' mm')
ax.set_xlabel("H [-]")
ax.set_ylabel("r [mm]")
fig.colorbar(cax)

def animate(i):
    cax.set_array(np.abs(HHG_onscreen[i,k_z0,:,:].T)) #G[:-1, :-1, i].flatten())
    ...
    ax.set_title("I0 = {:.2E}".format(1e-4*units.INTENSITYau*I0_grid[i]) + " W/cm2, z0={:.2E}".format(1e3*z0_grid[k_z0]) + ' mm')
...
anim.save(os.path.join('anims',anim_name), writer=writervideo)
