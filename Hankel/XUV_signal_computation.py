...
def Phi_2pi_decider(Phi):
    eps0 = np.finfo(float).eps
    return ((np.imag(Phi) <= 8.0*eps0 ) and
            (
            ((np.real(Phi) % (2.0*np.pi)) <= 16.0*eps0)
            or
            ( abs( (np.real(Phi) % (2.0*np.pi)) - 2.0*np.pi) <= 16.0*eps0)
            ))
...
def compute_S1_abs(pressure, zeta, ionisation_ratio, l1, Horder, parameters, include_absorption = True):
    ...
    ## delta k1
    delta_k1 = Horder * k0 * (0.5*pressure*N_atm*( (polarisability_IR - polarisability_XUV(Horder)) - ionisation_ratio*plasma_constant) - zeta )
    ...
    # add absorption
    if include_absorption: delta_k1 = delta_k1 + 1j*Horder*k0 * pressure * beta_factor_atm(Horder)
    ...
    if include_absorption:
        return S1, delta_k1, L_coh, delta_k1, L_abs(Horder)
    else:
...
def compute_Phi_abs(pressure, zeta, l1, xi, ionisation_ratio, Horder, parameters, include_absorption = True):
    ...
    # add absorption
    if include_absorption: Phi = Phi + 1j*Horder*l1*k0 * pressure * beta_factor_atm(Horder)
...
def eta_opt(Horder, parameters):
    ...
    return (omegaSI**2) *units.eps0*units.elmass*delta_susc(Horder*omegaSI)/(units.elcharge**2)
...
def zeta_single_segment(pressure, Horder, ionisation_ratio, parameters):
    ...
    zeta =  0.5 * pressure * N_atm * ( delta_susc(Horder*omegaSI) - ionisation_ratio*plasma_constant) 
...
def xi_calc_pm(delta_phi, pressure, l1, zeta, ionisation_ratio, Horder, parameters):
    ...
    xi = (1/zeta) * (0.5 * pressure * N_atm * (delta_susc(Horder*omegaSI) - ionisation_ratio*plasma_constant) -
                     2.0 * delta_phi / (k0*l1)
                     ) - 1.0
...
# def zeta_opt(pressure, Horder, parameters):
...
#     return -0.5*pressure*N_atm*polarisability_XUV(Horder)
