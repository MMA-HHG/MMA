...
with h5py.File(file_path, 'r') as InputArchive:
    ...
    # Nref = 1e6*InputArchive['/inputs/medium_effective_atmospheric_density_of_neutral_molecules'][()]
    ...
    nXUV = 1 - pressure*Nref*units.r_electron_classical*(mn.ConvertPhoton(omega0,'omegaSI','lambdaSI')**2)*f1/(2.0*np.pi)
    xxx = Nref*units.r_electron_classical*(mn.ConvertPhoton(omega0,'omegaSI','lambdaSI')**2)/(2.0*np.pi)

    lamdagrid = np.linspace(8e-9,35e-9,2000)
    cmprXUV = []
    for k1 in range(len(lamdagrid)):
        cmprXUV.append(((-Nref*units.r_electron_classical*lamdagrid[k1]**2)/(np.pi))*XUV_index.getf('Ar', mn.ConvertPhoton(lamdagrid[k1], 'lambdaSI', 'eV'))[0])

    cmprXUV = np.asarray(cmprXUV)

    plt.plot(lamdagrid,cmprXUV, linewidth=0.2)
    plt.savefig('suscXUV.png', dpi = 600)
    plt.show()
