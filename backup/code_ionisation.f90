...
MODULE Complex_rotation
  ...
  ! This module is used for Complex rotation. It does the following procedure
  ! 1- Rescale the table
  ! 2- A routine to serach the intensity
  ...
  INTEGER, SAVE            :: JLOTI, JLOTIP1                          ! Integer for Intensity Index
  DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE :: PPT_TABLE, CPR_TABLE 
...
CONTAINS
  ...
  SUBROUTINE RESCALE_TABLE_CPR
    ...
    DOUBLE PRECISION            :: dum, dumr1, dumr2
    INTEGER                     :: i
    INTEGER			:: DIMENSION_CPR
    INTEGER			:: k1, k2, k3, k4
    LOGICAL			:: dumlog
    ...
    ! Rescale the Table
    open(UNIT=3,FILE='Ar0.dat',FORM="FORMATTED",action='read');
    eps1 = 1.0d-14; ! the actual precision of the calculations, the precision could make the rate negative...
    ...
    dumlog = .true.
    k1 = 0
    DO WHILE ( dumlog .eq. .true. )
       read(unit=3,fmt=*) intensity, dum, ionisation_rate
       if( abs(ionisation_rate) .le. eps1 ) then
	 k1 = k1 + 1
         dumr1 = ionisation_rate
	 dumr2 = intensity
       else
	 dumlog = .false.
       endif
    ...
    DO i = 1, DIMENSION_CPR
       read(unit=3,fmt=*) intensity, dum, ionisation_rate !the organisation:  ||field omplitude | real part of eigenenrgy | imag. part of eigenerg. || all in atomic units
       if( i .le. k1 ) ionisation_rate = intensity * dumr1 / dumr2 ! linear growth for small intensities below the precision of CR
       intensity = intensity * intensity * field_intensity_au !rescale intensity intensity from atomic units to W/cm^2 (not SI)
       ionisation_rate = -2.0d0*ionisation_rate ! ionisation rate computed from the eigenergy using complex rotation
...
!       ionisation_rate = ionisation_rate_PPT(intensity)
  ...
  !----------------------------------------
  ! 2- Find the intensitty and Interpolate
...
!       READ(5,*)
    ...
    ENDIF
    IF (jlotip1.LT.1) jlotip1=1
    mpa=        ((intensity-cpr_table(jlotip1,1))*cpr_table(jlotip1+1,3)+(cpr_table(jlotip1+1,1)-intensity)*cpr_table(jlotip1,3))*intensity_step_inv
    jmin=MIN(jloti,jlotip1)
    jmax=MAX(jloti,jlotip1)
