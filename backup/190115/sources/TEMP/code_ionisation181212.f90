...
CONTAINS
  ...
  SUBROUTINE FILL_TABLE(THEORY)
    ...
    ALLOCATE(PPT_TABLE(DIMENSION_PPT, 3),dumvect(DIMENSION_PPT))
    ...
    IF(my_rank.EQ.0) dumvect = PPT_TABLE(:, 1)
    CALL MPI_BCAST(dumvect,DIMENSION_PPT,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
    IF(my_rank.NE.0)  PPT_TABLE(:, 1) = dumvect

    IF(my_rank.EQ.0) dumvect = PPT_TABLE(:, 2)
    CALL MPI_BCAST(dumvect,DIMENSION_PPT,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
    IF(my_rank.NE.0)  PPT_TABLE(:, 2) = dumvect


    IF(my_rank.EQ.0) dumvect = PPT_TABLE(:, 3)
    CALL MPI_BCAST(dumvect,DIMENSION_PPT,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
    IF(my_rank.NE.0)  PPT_TABLE(:, 3) = dumvect
...
MODULE Complex_rotation
  ...
  IMPLICIT NONE
  DOUBLE PRECISION, SAVE   :: INTENSITY_STEP, intensity_factor
...
!    DOUBLE PRECISION            :: intensity_factor
    ...
    DIMENSION_CPR = filelength('rates_atomic.dat')
...
!    
   ...
   IF(my_rank.EQ.1) THEN
       OPEN (UNIT = 2, FILE = 'ionisation_table1.dat', STATUS = 'unknown')
       DO i = 1, dimension_CPR
          WRITE(2, '(3(2x, e12.5))') CPR_TABLE(i, 1),CPR_TABLE(i, 2),CPR_TABLE(i, 3)
       ENDDO
       CLOSE(2)
