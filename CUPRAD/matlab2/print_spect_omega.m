function print_spect_omega(inpath,outpath,format,resolution,w0cm,tpfs,lambdanm,n0,omegaminhz,omegamaxhz,intmin,plotvecz);
...
filename = strcat(outpath,strcat('spect_omega_zgrid.dat'));
fileID_grid = fopen(filename,'w');
...
% for n=1:4
	...
	if ismember(n,plotvecz)
			fprintf(fileID_grid,'%E \n',z);
			filename = strcat(outpath,strcat('spect_omega_',num2str(n),'.dat'));
			...
			fclose(fileID);
	end; if n == m % this construct ensures the last run
			filename = strcat(outpath,'spect_omega_end.dat');
			fileID = fopen(filename,'w');
			for k4 = 1: (length(x)-1)
			fprintf(fileID,'%E \t %E \t %E \n',x(k4), y(k4), v(k4));
			end
...
%	if n == 1
%			filename = strcat(outpath,'spect_omega_1.dat');
%			fileID = fopen(filename,'w');
%			for k4 = 1: (length(x)-1)
%			fprintf(fileID,'%E \t %E \t %E \n',x(k4), y(k4), v(k4));
%			end
