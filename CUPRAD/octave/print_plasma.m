function print_plasma(inpath,outpath,format,resolution,w0cm,tpfs,lambdanm,n0,tminfs,tmaxfs,rmaxmm);
...
filter = sprintf('%s%s',inpath,'*plasma*');
files = dir(filter);
[m,n] = size(files);
k=1;
l=1;
for n=1:m
    ...
    graphtitle = sprintf('%s%0.4g%s','z = ',z,' m');...
        ...
        set(gca,'FontSize',10);
    if k==10
        tiffile = sprintf('%s%s%d',outpath,'plasma',l/10);
...
if k~=1
    tiffile = sprintf('%s%s%d',outpath,'plasma',(l+10-k)/10);
    saveas(gcf,sprintf('%s%s',tiffile,'.pdf'));
    print(format,resolution,tiffile);
    close all;
end;
