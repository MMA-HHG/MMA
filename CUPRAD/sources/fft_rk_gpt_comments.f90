MODULE fft
    ...
    ! \brief FFTW plans for various forward and backward transformations.
    INTEGER(8) :: plan_forward1, plan_forward_erk, plan_backward_erk, plan_backward2, plan_spec, plan_p, plan_j, plan_pharm
    ! \brief Inverse of the dimension size for normalization.
    REAL(8) :: diminv
  
  CONTAINS
  
    ! \brief Initialize FFT plans and MPI communications.
    ! 
    ! This subroutine sets up the necessary FFTW plans for forward and backward transformations
    ! and initializes persistent MPI communication channels for data transposition.
    SUBROUTINE fft_init
      IMPLICIT NONE
      ...
      ! Initialize the schema for MPI communications
      DO j = 1, num_proc
        schema(j) = j
      ENDDO
  
      ! Determine the power of 2 for the number of processors
      power = 0
      DO j = 1, 16
      ...
      ! Allocate and initialize arrays for process-specific data
      ...
      ! Allocate arrays for the field data and temporary storage
      ...
      ! Define MPI types for subarray communications
      ...
      ! Allocate MPI requests for send and receive operations
      ...
      ! Initialize MPI send and receive requests
      DO j = 1, num_proc - 1
      ...
      ! Create FFTW plans for forward and backward transformations
      m = dim_r_local
      n = dim_t
      s = 1
    ...
    ! \brief Perform bitwise reversal on the vector for FFT.
    !
    ! This subroutine performs bitwise reversal on the input vector, which is a
    ! necessary step in the FFT algorithm.
    ...
    ! \brief Perform forward FFT and apply non-linear steps if specified.
    !
    ! This subroutine executes the forward FFT plan and, if specified, applies non-linear steps
    ! based on the value of \p nlinstep. It also handles the data transposition and MPI communications.
    SUBROUTINE fft_forward_inplace(nlinstep)
      IMPLICIT NONE
  
      ! \param nlinstep Logical flag to determine if non-linear steps should be applied.
      LOGICAL :: nlinstep
      INTEGER(4) :: k, l, i
      ...
      ! Perform MPI communications for transposed data
      DO i = 1, num_proc - 1
    ...
    ! \brief Perform backward FFT and transpose the array back.
    !
    ! This subroutine executes the backward FFT plan, transposes the array back to its original
    ! layout, and handles the MPI communications for data consistency.
    ...
    ! \brief Destroy FFTW plans.
    !
    ! This subroutine releases the resources associated with the FFTW plans.
    SUBROUTINE fft_exit
      IMPLICIT NONE
