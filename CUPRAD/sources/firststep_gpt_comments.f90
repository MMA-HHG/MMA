! This Fortran module handles the initialization and calculation of propagation
! parameters for a simulation involving various ionization models, propagation 
! operators, and density modifications. It includes subroutines for computing 
! local wave numbers, time propagators, and Crank-Nicolson propagators, as well as 
! initializing simulation parameters and preparing necessary data structures.
!
! \file first_step.f90
! \brief Module for initialization and calculation of propagation parameters.
! \author Your Name
! \date 2024-05-29
...
MODULE first_step
 ...
 CONTAINS
   ...
   ! \brief Subroutine to calculate the local wave number and reduced wave number
   ! \param omega_local Local frequency
   ! \param density_mod_local Local density modification
   ! \param komega_local Local wave number (input/output)
   ! \param komega_red_local Reduced wave number (output)
   SUBROUTINE calc_komega_local(omega_local, density_mod_local, komega_local, komega_red_local)
     USE ppt
     IMPLICIT NONE
     REAL(8) :: cnorm              !< Normalized vacuum light velocity
     REAL(8) :: omega_local        !< Local frequency
     REAL(8) :: density_mod_local  !< Local density modification
     COMPLEX(8) :: chi_local       !< Local susceptibility
     COMPLEX(8) :: komega_local    !< Local wave number
     COMPLEX(8) :: komega_red_local !< Reduced wave number
 
     ! Compute normalized vacuum light velocity
     cnorm = c_light * pulse_duration * 1.D-15 / four_z_Rayleigh
 
     ! Compute local susceptibility and wave numbers
   ...
   ! \brief Subroutine to calculate the time propagator
   SUBROUTINE calc_time_propagator
     USE ppt
     IMPLICIT NONE
 
     INTEGER(4) :: j                !< Loop index for time dimension
     INTEGER(4) :: k                !< Loop index for spatial dimension
     COMPLEX(8) :: komega_local     !< Local wave number
     COMPLEX(8) :: komega_red_local !< Reduced wave number
     REAL(8) :: delta_zh            !< Half of the spatial step size
 
     delta_zh = 0.5D0 * delta_z
 
     ! Calculate propagator for each time and space step
     IF (dim_t_start(num_proc) < dim_th) THEN
     ...
     ! Select propagation method based on switch_T
     ...
     CASE (1)
       DO k = 1, dim_r
         ...
         DO j = 1, dim_t
           IF ((j >= dim_t_start(num_proc)) .AND. (j <= dim_t_end(num_proc))) op_t_inv_cn(k, j) = rek0 / REAL(komega_local, 8)
           IF ((k >= dim_r_start(num_proc)) .AND. (k <= dim_r_end(num_proc))) THEN
     ...
     CASE (2)
       DO k = 1, dim_r
         ...
         CALL calc_komega_local(omega, density_mod(k), komega_local, komega_red_local)
         DO j = 1, dim_th
           IF ((j >= dim_t_start(num_proc)) .AND. (j <= dim_t_end(num_proc))) op_t_inv_cn(k, j) = CMPLX(omega / (omega_uppe + k_t * REAL(j - 1, 8)), 0.D0, 8) * rek0 / REAL(komega_local, 8)
           IF ((dim_th + j >= dim_t_start(num_proc)) .AND. (dim_th + j <= dim_t_end(num_proc))) op_t_inv_cn(k, dim_th + j) = CMPLX(omega / (omega_uppe + k_t * REAL(j - dim_th - 1, 8)), 0.D0, 8) * rek0 / REAL(komega_local, 8)
           IF ((k >= dim_r_start(num_proc)) .AND. (k <= dim_r_end(num_proc))) THEN
     ...
     CASE (3)
       DO k = 1, dim_r
         DO j = 1, dim_th
           ...
           IF ((j >= dim_t_start(num_proc)) .AND. (j <= dim_t_end(num_proc))) op_t_inv_cn(k, j) = rek0 / komega_local
           IF ((k >= dim_r_start(num_proc)) .AND. (k <= dim_r_end(num_proc))) THEN
           ...
           IF ((dim_th + j >= dim_t_start(num_proc)) .AND. (dim_th + j <= dim_t_end(num_proc))) op_t_inv_cn(k, dim_th + j) = rek0 / komega_local
           IF ((k >= dim_r_start(num_proc)) .AND. (k <= dim_r_end(num_proc))) THEN
   ...
   ! \brief Subroutine to calculate the Crank-Nicolson propagator
   SUBROUTINE calc_cn_propagator
     ...
     delta_zh = 0.5D0 * delta_z    !< Half of the spatial step size
 
     ! Initialize hfac array if switch_T equals 3
     ...
     ! Calculate diagonal, upper diagonal, and lower diagonal elements for Crank-Nicolson scheme
     ...
     DO k = dim_t_start(num_proc), dim_t_end(num_proc)
       ...
       ENDDO
       D(dim_r, k) = CMPLX(1.D0, 0.D0, 8) ! Hadley boundary in r
     ENDDO
 
     RETURN
   END SUBROUTINE calc_cn_propagator
 
   ! \brief Subroutine to initialize the simulation parameters and data structures
   SUBROUTINE initialize
     ...
     INTEGER(4) :: j, k, help, k1, pos    !< Loop indices and auxiliary variables
     REAL(8) :: absorb_factor             !< Absorption factor
     CHARACTER*255 :: filename            !< Filename
     INTEGER(HID_T) :: file_id, group_id  !< HDF5 file and group identifiers
     INTEGER :: error                     !< Error code
     REAL(8), ALLOCATABLE :: real_e(:,:), imag_e(:,:) !< Real and imaginary parts of the electric field
 ...
 999   CONTINUE
     ...
     ! Open HDF5 interface and read inputs
     CALL h5open_f(error)
     ...
     four_z_Rayleigh = 1.D-2 * four_z_Rayleigh
     ...
     ! Check for density modifications and pre-ionisation
     CALL h5lexists_f(file_id, density_mod_grpname, apply_density_mod, error)
     IF (apply_density_mod) CALL init_density_mod(file_id)
 
     CALL h5lexists_f(file_id, pre_ionised_grpname, apply_pre_ionisation, error)
     IF (apply_pre_ionisation) CALL init_pre_ionisation(file_id)
 
     ! Allocate and initialize density modification array
     ...
     delta_t = lt / REAL(dim_t, 8)         !< Temporal step size
     delta_r = lr / REAL(dim_r, 8)         !< Spatial step size
     delta_t_inv = 1.D0 / delta_t          !< Inverse of the temporal step size
     tlo = -0.5D0 * lt                     !< Lower bound of time
     count = 0                             !< Counter variable
     k_t = 8.D0 * DATAN(1.D0) / lt         !< Temporal frequency step size
     dim_th = dim_t / 2                    !< Half of the temporal dimension
     DO j = 1, dim_t
       komega_red(j) = komega(j) - CMPLX(rekp * (k_t * REAL(j - dim_th - 1, 8) + omega_uppe), 0.D0, 8)
     ENDDO
 
     ! Determine help variable based on switch_T
     SELECT CASE (switch_T)
     CASE (1)
     ...
     ! Allocate necessary arrays and initialize boundary conditions
     ...
     ! Select Kerr model based on switch_dKerr
     ...
     ! Allocate and initialize electric field arrays
     ...
     efield_factor = SQRT(critical_power * 1.D9 * c_light * 4.D0 * PI * 1.D-7 / (4.D0 * PI * beam_waist ** 2 * 1.D-4 * 2.D0 * n0_indice)) * 2.D0 !< Normalization factor for electric field
     ...
     plasma_normalisation_factor_m3 = rhoc_cm3_phys / (4.0D0 * PI ** 2 * (beam_waist ** 2) / ((lambdanm * 1.0D-7) ** 2)) !< Plasma normalization factor in m^(-3)
     plasma_normalisation_factor_m3 = 1.0D6 * plasma_normalisation_factor_m3 !< Plasma normalization factor in m^(-3)
     tps = pulse_duration * 1.D-15   !< Pulse duration in seconds
     w0m = beam_waist * 1.D-2        !< Beam waist in meters
 
     Nz_points = CEILING(proplength / outlength) + 1 !< Expected number of HDF5 output points along z
     Nz_points_Efield = CEILING(proplength / outlength_Efield) + 1 !< Expected number of HDF5 output points for E-field along z
     ...
     IF (my_rank .EQ. 0) THEN
       ...
       CALL create_1D_dset_unlimited(group_id, 'zgrid_dz_CU', (/REAL(z, 4)/), 1)
       CALL h5_add_units_1D(group_id, 'zgrid_dz_CU', '[C.U.]')
       CALL create_1D_dset_unlimited(group_id, 'zgrid_dz_SI', (/REAL(four_z_Rayleigh * z, 4)/), 1)
       CALL h5_add_units_1D(group_id, 'zgrid_dz_SI', '[SI]')
       CALL create_1D_dset_unlimited(group_id, 'dz', (/REAL(delta_z, 4)/), 1)
       CALL h5_add_units_1D(group_id, 'dz', '[C.U.]')
       CALL create_1D_dset_unlimited(group_id, 'maxphase', (/-1.0/), 1)
     ...
     ! Select the ionization model based on switch_rho
