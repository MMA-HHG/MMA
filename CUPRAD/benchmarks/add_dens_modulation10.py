...
# prepare zgrid
zmin = 0.
zmax = 0.01 # float(sys.argv[1]) # 1e-2
dz = 100e-8
...
# image.sf[0].args = [zgrid, Gauss_train_stride(zgrid, 2.0, a_Gauss, 1.5*a_Gauss, 0.01/10, 10)]  


# pp.plot_preset(image)  

# print(np.trapz(Gauss_train_stride(zgrid, 2.0, a_Gauss, 1.5*a_Gauss, 0.01/10, 10),zgrid)/zmax)
...
with h5py.File('results.h5', 'a') as f:
    ...
    mn.adddataset(f, 'density_mod/table', Gauss_train_stride(zgrid, 2.0, a_Gauss, 1.5*a_Gauss, 0.01/10, 10), '[-]')
