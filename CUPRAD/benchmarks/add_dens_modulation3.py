...
# image.sf[0].args = [zgrid, Gaussian_train(zgrid, 1.0, a_Gauss, [zmax/4., 3.*zmax/4.])]  


# pp.plot_preset(image)  

print(np.trapz(Gaussian_train(zgrid, 1.0, a_Gauss, [zmax/4., 3.*zmax/4.]),zgrid)/zmax)


# add to file
with h5py.File('results.h5', 'a') as f:
    mn.adddataset(f, 'density_mod/zgrid', zgrid, '[m]')
    mn.adddataset(f, 'density_mod/table', Gaussian_train(zgrid, 1.0, a_Gauss, [zmax/4., 3.*zmax/4.]), '[-]')
