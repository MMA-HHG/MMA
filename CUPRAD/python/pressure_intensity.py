...
# be further extended by storing also figure sources etc.
    ...
    # Here we loop over all result files in the destiantion folder.
        ...
        with h5py.File(file_path, 'r') as InputArchive:
            ...
            for k1 in range(res.Nr):
                ...
                plasma_end_pmap[k_press,k1,:] = np.interp(zgrid_ref,res.zgrid, plasma_end[k1,:])
                plasma_tmax_pmap[k_press,k1,:] = np.interp(zgrid_ref,res.zgrid,plasma_tmax[k1,:])
    ...
    map2 = ax1.contour(pressure_list_mbar, 1e6*rgrid_ref, Cutoff_tmax_pmap[:,:,-1].T, Horders, colors = "black")
