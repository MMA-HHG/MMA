function [fullarray, normfullarray, cm3fullarray, SIfullarray, normfactor] = analyse_plasma(inpath,outpath,format,resolution,w0cm,tpfs,lambdanm,n0,tminfs,tmaxfs,rmaxmm,rhoccm3,rhontcm3);
...
for n=1:m
    ...
    % return plasam arrays
    [Nx,Ny] = size(arr);
    Nx = floor((Nx+1)/2);
    fullarray = arr;
    fullarray = fullarray(1:Nx,:);
    fullarray = flip(fullarray,1);
    normfactor = (rhoccm3/(rhontcm3*4*3.1415^2*w0cm^2/(lambdanm*1.e-7)^2));
    normfullarray = (rhoccm3/(rhontcm3*4*3.1415^2*w0cm^2/(lambdanm*1.e-7)^2))*fullarray;
    cm3fullarray = rhoccm3/(4*3.1415^2*w0cm^2/(lambdanm*1.e-7)^2)*fullarray;
    SIfullarray = 1e6*cm3fullarray;
    ...
    graphtitle = sprintf('%s%0.4g%s','z = ',z,' m');...
        ...
        set(gca,'FontSize',10);
    if k==2
        tiffile = sprintf('%s%s%d',outpath,'myplasma',l/2);
...
if k~=1
    tiffile = sprintf('%s%s%d',outpath,'myplasma',(l+2-k)/2);
    saveas(gcf,sprintf('%s%s',tiffile,'.pdf'));
    print(format,resolution,tiffile);
    close all;
end;
