function print_test(inpath,outpath,format,resolution,w0cm,tpfs,lambdanm,n0,PcrGw,rhoccm3,rhontcm3,sigmak,KK,zminm,zmaxm,rmaxmm,tminfs,tmaxfs,intmintwcm2,intmaxtwcm2,omegaminhz,omegamaxhz);
...
subplot(5,2,6);%subplot('Position',[0.6 0.46 0.35 0.13]);
sxn=999;
syn=1000;
xn=zeros(sxn,1);
for n=1:floor(sxn/2)
...
saveas(gcf,sprintf('%s%s',outpath,'propagation.pdf'));
tiffile = sprintf('%s%s',outpath,'propagation');
