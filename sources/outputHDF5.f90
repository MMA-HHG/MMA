...
CONTAINS
  ...
  SUBROUTINE HDF5_out
	...
	CALL h5screate_simple_f(field_dimensions, dims, dataspace, error, maxdims) 


	!Modify dataset creation properties, i.e. enable chunking

	CALL h5pcreate_f(H5P_DATASET_CREATE_F, crp_list, error)
	CALL h5pset_chunk_f(crp_list, field_dimensions, dimsc, error)

        CALL h5pset_dxpl_mpio_f(plist_id, H5FD_MPIO_INDEPENDENT_F, error) ! allow MPI access - should it be here?

	!Create dataset
	CALL h5dcreate_f(file_id, dsetname, H5T_NATIVE_FLOAT, dataspace, dset_id, error, crp_list)
	

	!chunk data for each worker
	offset = (/1,dim_r_start(num_proc),0/)
	...
	CALL h5sclose_f(dataspace, error) ! now, all prerequisities for writing are done


	! Now I use hyperslab to write chunked data
	! reallocate data
