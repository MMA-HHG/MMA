...
999    CONTINUE
    ...
    CALL readreal(group_id, 'rekp',rekp)
    CALL readreal(group_id, 'c3',c3)
    CALL readreal(group_id, 'c5',c5)
    CALL readreal(group_id, 'gamma1',gamma1)
    CALL readreal(group_id, 'gamma2',gamma2)
    CALL readreal(group_id, 'muk',muk)
    CALL readreal(group_id, 'beta_inv_2KK',beta_inv_2KK)
    CALL readint(group_id,'KK',KK)
    CALL readreal(group_id, 'rho0',rho0)
    CALL readreal(group_id, 'nu',nu)
    CALL readreal(group_id, 'alpha',alpha)
    CALL readreal(group_id, 'alphaquad',alphaquad)
    CALL readreal(group_id, 'rhoat_inv',rhoat_inv)
    CALL readreal(group_id, 'xdk',xdk)
    CALL readreal(group_id, 'tdk',tdk)
    CALL readreal(group_id, 'raman',raman)
    CALL readreal(group_id, 'omega',omega)
    ALLOCATE(komega(dim_t),komega_red(dim_t))
    CALL read_array_complex_dset(group_id,'komega',komega,dim_t)
    CALL readint(group_id, 'NN',NN)
    CALL readreal(group_id, 'eta1',eta1)
    CALL readreal(group_id, 'eta2',eta2)
    CALL readreal(group_id, 'lt',lt)
    CALL readreal(group_id, 'lr',lr)
    CALL readreal(group_id, 'proplength',proplength)
    CALL readreal(group_id, 'outlength',outlength)
    CALL readreal(group_id, 'delta_z',delta_z)
    CALL readreal(group_id, 'z',z)
    CALL readreal(group_id, 'z_out',z_out)
    CALL readreal(group_id, 'rfil',rfil)
    CALL readint(group_id,'switch_rho', switch_rho)
    CALL readint(group_id,'switchKerr', switch_dKerr)
    CALL readint(group_id,'switch_T',switch_T)
    CALL readint(group_id,'absorb',absorb)
    CALL readreal(group_id, 'increase',increase)
    CALL readreal(group_id, 'decrease',decrease)
    CALL readint(group_id,'rhodist',rhodist)
    CALL readreal(group_id, 'timelimit',timelimit)
    CALL readreal(group_id, 'photenergy',photon_energy)
    CALL readreal(group_id, 'pulsedurat',pulse_duration)
    CALL readreal(group_id, 'critpower',critical_power)
    CALL readreal(group_id, 'beam_waist',beam_waist)
    CALL readreal(group_id, 'ionpot',ionisation_potential)
    CALL readreal(group_id, 'rescharge',residue_charge)
    CALL readreal(group_id, 'n0_indice',n0_indice)
    CALL readreal(group_id, 'critdens',critical_density)
    CALL readreal(group_id, 'atomdens',atomic_density)
    CALL readreal(group_id, 'reducmass',reduced_mass)
    CALL readint(group_id,'angmom',angular_momentum)
    CALL readint(group_id,'KKp',KKp)
    CALL readreal(group_id, 'beta_inv_2KKp',beta_inv_2KKp)
    CALL readreal(group_id, 'mukp',mukp)
    CALL readreal(group_id, 'beta_inv_2',beta_inv_2)
    CALL readreal(group_id, 'mu',mu)
    CALL readint(group_id,'KKpp',KKpp)
    CALL readreal(group_id, 'beta_inv_2KKpp',beta_inv_2KKpp)
    CALL readreal(group_id, 'mukpp',mukpp)
    CALL readreal(group_id, 'eti_ref',eti_ref)
    CALL readreal(group_id, 'exp_ref',exp_ref)
    CALL readreal(group_id, 'alpha1',alpha1)
    CALL readreal(group_id, 'alpha2',alpha2)
    CALL readreal(group_id, 'alphah',alphah)
    CALL readreal(group_id, 'rhosat',rhosat)
    CALL readbool(group_id, 'finished',finished)
    CALL readreal(group_id, 'omega_uppe',omega_uppe)
    CALL readreal(group_id, 'gamma1e',gamma1e)
    CALL readreal(group_id, 'nuO2',nuO2)
    CALL readreal(group_id, 'nuN2',nuN2)
    CALL readreal(group_id, 'T_init_eV_phys',T_init_eV_phys)
    CALL readreal(group_id, 'nukB',nukB)
    CALL readreal(group_id, 'nucp',nucp)
    CALL readreal(group_id, 'nucO2',nucO2)
    CALL readreal(group_id, 'nucN2',nucN2)
    CALL readreal(group_id, 'rhoat_N2_inv',rhoat_N2_inv)
    CALL readreal(group_id, 'ionpotN2',ionisation_potential_N2)
    CALL readreal(group_id, 'rescharge_N2',residue_charge_N2)
    CALL readreal(group_id, 'atomdens_N2',atomic_density_N2)
    CALL readint(group_id,'angmom_N2',angular_momentum_N2)
    CALL fft_init
